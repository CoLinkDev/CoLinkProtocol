...
|-------|------|-------------|
| nonce | string | Cryptographically random string. The reference implementation uses a UUID v4 without hyphens (32 lowercase hex characters). Implementations MAY use any format as long as it is generated by a cryptographically secure random source and encoded as a valid JSON string. |
...
The **initiator** is the peer that opened the outbound WebSocket connection (TCP client). The **receiver** is the peer that accepted the inbound connection (TCP server). This role assignment is fixed for the lifetime of the connection and governs message ordering in the flows below.
