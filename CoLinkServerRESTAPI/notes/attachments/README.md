...
无论附件由客户端显式删除还是由服务器自动清理，其 `attachmentId` 都 MUST 在当前账户内永久保留为墓碑，直到该账户被删除。后续上传不得复用该 ID；元数据和内容查询仍返回 `6005 attachment not found`。此规则防止客户端缓存把新内容误认为已删除的旧附件。墓碑不计入用户存储配额。
