...
If the `attachmentId` already exists or was previously used in the same account, the server returns `6011 attachment ID unavailable`. When the upload outcome is uncertain, the client MUST first query the attachment metadata. If `kind`, `size`, and `sha256` all match the file being uploaded, the upload is considered successful. The client retries the upload with the original ID only if the attachment does not exist. If that upload returns `6011`, or if existing metadata does not match, the client MUST replace the ID as defined in [Handling Uncertain Request Outcomes](../notes-and-tags/README.md#handling-uncertain-request-outcomes). The existing attachment MUST NOT be overwritten.

## Errors

| Code | Message | HTTP Status | Description |
|------|---------|-------------|-------------|
| 4001 | invalid request body | 400 | Multipart form missing required parts |
| 4002 | invalid parameter | 400 | `attachmentId`, `kind`, or `sha256` is malformed, or `fileName` is invalid UTF-8 or contains control characters |
| 6007 | note storage limit reached | 413 | The file exceeds the maximum attachment size or the account's Notes storage quota is reached |
| 6010 | attachment checksum mismatch | 422 | Uploaded content SHA-256 does not match the `sha256` in the request |
| 6011 | attachment ID unavailable | 409 | `attachmentId` is already reserved by an active attachment or tombstone in this account |
