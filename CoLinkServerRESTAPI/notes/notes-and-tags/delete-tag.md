...
|------|---------|-------------|-------------|
| 4002 | invalid parameter | 400 | `tagId` is not a valid UUID v4, or `baseRevision` is missing or not a valid integer |
| 6002 | revision conflict | 412 | `baseRevision` does not match the current tag revision |
| 6003 | tag not found | 404 | Tag does not exist, has already been deleted, or does not belong to the current account |
