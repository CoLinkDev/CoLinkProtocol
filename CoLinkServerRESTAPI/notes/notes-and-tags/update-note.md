...
这是完整替换操作，不采用字段级合并。服务器 MUST 使用 `noteId + userId + baseRevision` 做条件更新；仅在版本匹配时写入 `revision + 1`。版本不匹配返回 `6002 revision conflict`，响应 `data` 为 `null`，客户端再获取当前笔记执行三方合并。缺少或无法解析 `baseRevision` 时返回 `4002 invalid parameter`。
